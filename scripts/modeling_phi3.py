...
class Phi3FlashAttention2(Phi3Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        bsz, q_len, _ = hidden_states.size() # In CnP=1, q_len = suffix length
        ...
        if past_key_value is not None:
            ...
            # If CnP=1: this makes kv_seq_len = suffix length + prefix length
            # If CnP=0: this makes kv_seq_len = total prefill query length in prefill,  (total prefill + decode so far) length in decode
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        ...
        if past_key_value is not None:
            ...
            cache_kwargs = {"sin": sin, "cos": cos}  # Specific to RoPE models
            # works for CnP=1 too. When we store KV, Key with RoPE embedded is stored.
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
    ...
    # For batch_size=1: FlashAttention takes care of correctly computing suffix query against total_kv attention
...
class Phi3Model(Phi3PreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            batch_size, seq_length = input_ids.shape[:2] # if CnP=1 this is suffix length
        ...
        if self._attn_implementation == "flash_attention_2":
            # 2d mask is passed through the layers. attn mask is only kept if there is atleast one 0 in it, or else its set to None
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if caching_and_prefill=1, this should return suffix's intermediate hs, attns and past_KV UPDATED to total_len till this layer_idx
                layer_outputs = decoder_layer(
                    hidden_states, 
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    past_key_value=past_key_values, #if caching_and_prefill=1 this is the mutable prefix KV you pass in, which is updated to final KV with each iteration.
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    caching_and_prefill=caching_and_prefill,
                )
...
class Phi3ForCausalLM(Phi3PreTrainedModel):
    ...
    def forward(
        ...
        if(self.caching_strategy == 1 and past_key_values is None): # Prefix-caching AND Prefill Stage
            ...
            midpoint = seq_len // 2
            prefix_input_ids = input_ids[:, :midpoint]  # First half
            suffix_input_ids = input_ids[:, midpoint:]
            
            if(position_ids is not None):
                # Split position_ids into prefix and suffix
                prefix_position_ids = position_ids[:, :midpoint]
                suffix_position_ids = position_ids[:, midpoint:]
            if(attention_mask is not None):
                # Split attention_mask into prefix and suffix
                prefix_attention_mask = attention_mask[:, :midpoint]
                suffix_attention_mask = attention_mask[:, midpoint:]
            
            if position_ids is not None and attention_mask is not None:
                assert prefix_input_ids.shape == prefix_position_ids.shape == prefix_attention_mask.shape, \
                    f"Prefix shapes mismatch: {prefix_input_ids.shape}, {prefix_position_ids.shape}, {prefix_attention_mask.shape}"
            ...
            output_prefix = self.model(
                ...
                position_ids=prefix_position_ids, # even if they are provided. I don't wanna split pos_ids and send -_-
            ...
            # This is where cache identifying mechanism should end
            ...
            # Do prefill for SUFFIX input - this should return suffix last_hs, prefix+suffix past_kv, suffix all_hs, suffix_attn
            outputs = self.model(
                input_ids=suffix_input_ids,
                attention_mask=suffix_attention_mask,
                position_ids=suffix_position_ids,
                past_key_values=prefix_key_values, # send prefix KV - use it to find prefix length.
                inputs_embeds=inputs_embeds,
                use_cache=use_cache,
                output_attentions=output_attentions,
            ...
            # Create a new BaseModelOutputWithPast with the modified last_hidden_state
